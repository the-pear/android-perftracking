...
public class ClassTrimmer {
    ...
    public ClassNode trim(ClassNode cn) {
        if ((!checkAnnotations(cn.visibleAnnotations)) || (!checkAnnotations(cn.invisibleAnnotations))) {
    ...
    private boolean checkAnnotation(AnnotationNode a) {
        if (a.desc.equals("Ljp/co/rakuten/sdtd/perf/core/annotations/Exists;")) {
            if (!exists(((Type)a.values.get(1)).getClassName())) {
                return false;
            }
        }
        else if (a.desc.equals("Ljp/co/rakuten/sdtd/perf/core/annotations/MinCompileSdkVersion;")) {
            if (_compileSdkVersion < (int)a.values.get(1)) {
                return false;
            }
        }
        else if (a.desc.equals("Ljp/co/rakuten/sdtd/perf/core/annotations/MaxCompileSdkVersion;")) {
            if (_compileSdkVersion > (int)a.values.get(1)) {
                return false;
            }
        }
