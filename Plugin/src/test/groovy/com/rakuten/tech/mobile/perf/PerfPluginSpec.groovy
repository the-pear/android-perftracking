...
class PerfPluginSpec {
    ...
    @Test void "should add runtime dependency to project"() {
        ...
        assert stubs.compileDependencies.find { it.startsWith("com.rakuten.tech.mobile.perf:performance-tracking") }
    }
...
class DebugExtension { def debug }
class ReleaseExtension { def release }
class TestDelegate {
    def repos = []
